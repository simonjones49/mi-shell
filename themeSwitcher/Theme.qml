...
Singleton {
    ...
    onPreviewIndexChanged: {
        if (previewIndex >= 0 && previewIndex < themes.length) {
            ...
            applyQutebrowserTheme(themes[previewIndex]);
        } else {
            applyKittyTheme(themes[currentIndex]);
            applyQutebrowserTheme(themes[currentIndex]);
        }
    }
    ...
    function setTheme(index) {
        if (index >= 0 && index < themes.length) {
            ...
            applyQutebrowserTheme(themes[index]);
    ...
    function applyQutebrowserTheme(t) {
        var quteConf = [
            "# Automatically generated by Quickshell mi-shell theme chooser",
            "c.colors.completion.category.bg = \"" + t.bgSurface + "\"",
            "c.colors.completion.category.fg = \"" + t.textPrimary + "\"",
            "c.colors.completion.item.selected.bg = \"" + t.bgSelected + "\"",
            "c.colors.completion.item.selected.fg = \"" + t.textPrimary + "\"",
            "c.colors.completion.item.selected.match.fg = \"" + t.accentCyan + "\"",
            "c.colors.completion.match.fg = \"" + t.accentPrimary + "\"",

            // Status Bar Background & General Text
            "c.colors.statusbar.normal.bg = \"" + t.bgBase + "\"",
            "c.colors.statusbar.normal.fg = \"" + t.textPrimary + "\"",
            "c.colors.statusbar.insert.bg = \"" + t.accentGreen + "\"",
            "c.colors.statusbar.insert.fg = \"" + t.bgBase + "\"",

            // === FIXES URL READABILITY ===
            "c.colors.statusbar.url.fg = \"" + t.textPrimary + "\"",
            "c.colors.statusbar.url.success.http.fg = \"" + t.textPrimary + "\"",
            "c.colors.statusbar.url.success.https.fg = \"" + t.accentGreen + "\"",
            "c.colors.statusbar.url.hover.fg = \"" + t.accentCyan + "\"",
            "c.colors.statusbar.url.warn.fg = \"" + t.accentPrimary + "\"",
            "c.colors.statusbar.url.error.fg = \"" + t.textPrimary + "\"",
            // =============================

            "c.colors.tabs.bar.bg = \"" + t.bgBase + "\"",
            "c.colors.tabs.even.bg = \"" + t.bgBase + "\"",
            "c.colors.tabs.odd.bg = \"" + t.bgBase + "\"",
            "c.colors.tabs.even.fg = \"" + t.textSecondary + "\"",
            "c.colors.tabs.odd.fg = \"" + t.textSecondary + "\"",
            "c.colors.tabs.selected.even.bg = \"" + t.bgSurface + "\"",
            "c.colors.tabs.selected.even.fg = \"" + t.textPrimary + "\"",
            "c.colors.tabs.selected.odd.bg = \"" + t.bgSurface + "\"",
            "c.colors.tabs.selected.odd.fg = \"" + t.textPrimary + "\"",
            "c.colors.webpage.bg = \"#ffffff\""
        ].join("\n");

        // Writes the python theme file and commands Qutebrowser to reload if it's currently running
        quteProc.command = ["sh", "-c",
        "printf '%s\\n' '" + quteConf + "' > $HOME/.local/share/qutebrowser/profiles/main/config/theme.py; " +
        "pgrep -x qutebrowser >/dev/null && qutebrowser --basedir $HOME/.local/share/qutebrowser/profiles/main :config-source 2>/dev/null || true"
        ];
        quteProc.running = true;
    }
    Process { id: saveProc; running: false }
    Process { id: kittyProc; running: false }
    Process { id: colorSchemeProc; running: false }
    Process { id: quteProc; running: false }
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                if (!isNaN(idx) && idx >= 0 && idx < root.themes.length) {
                    ...
                    root.applyQutebrowserTheme(root.themes[idx]);
    ...
    FileView {
        id: themesFile
        readonly property string themePath: "/etc/xdg/quickshell/mi-shell/themeSwitcher/themes.json"
        path: themePath
        onTextChanged: {
